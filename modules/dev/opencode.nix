    ...
    in
    {
      programs.opencode = {
        ...
        settings = {
          ...
          # MCP server visibility — overrides per-project opencode.json via
          # deep merge. Project configs (regenerated by switch-env) define the
          # servers; these knobs control which ones load and which tools the
          # default agent sees. Per-agent overrides can re-enable hidden tools.
          mcp = {
            "figma-dev-mode-mcp" = { enabled = false; };
